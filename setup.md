...
Во **втором терминале** — открыть тоннель:
...
Эту ссылку можно передать кому угодно — сайт будет работать пока оба процесса запущены.

> **Важно:** изображения из хранилища будут доступны только если `APP_URL` в `.env` совпадает с ngrok-адресом, либо они уже кешированы браузером. Для показа дизайна без галереи — всё работает сразу.
